...
export class App {
  ...
  updatePid(conf: IConfig, pidData: IPidDataInput) {
    ...
    if (!this.pids[pid]) {
      ...
      const memoryValues = [pidData.memory, ...this.pids[pid].memory].slice(
        0,
        conf.average_worker_resources_over_seconds,
      );
      const cpuValues = [pidData.cpu, ...this.pids[pid].cpu].slice(
        0,
        conf.average_worker_resources_over_seconds,
      );
  ...
  getAverageUsedMemory() {
    ...
    return Math.round(
      memoryValues.reduce((sum, value) => sum + value) / memoryValues.length,
    );
  }
